...
# Financial document processing functions
...
if app_mode == "Dashboard":
    ...
    if st.session_state.uploaded_data:
        if isinstance(st.session_state.uploaded_data, dict) and 'tables' in st.session_state.uploaded_data:
            for i, table in enumerate(st.session_state.uploaded_data['tables']):
                st.subheader(f"Table {i+1}")
                st.dataframe(pd.DataFrame(table[1:], columns=table[0]))
        elif isinstance(st.session_state.uploaded_data, pd.DataFrame):
            st.dataframe(st.session_state.uploaded_data)
        
        if isinstance(st.session_state.uploaded_data, pd.DataFrame):
            forecast = generate_cashflow_forecast(st.session_state.uploaded_data)
...
# Module: Accounting Automation
    ...
    with col1:
        ...
        if st.session_state.uploaded_data and isinstance(st.session_state.uploaded_data, pd.DataFrame):
    ...
    with col2:
        ...
        if st.session_state.uploaded_data and isinstance(st.session_state.uploaded_data, dict):
